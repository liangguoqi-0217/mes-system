...
const SpStockPost = {
  ...
  /* ---------------- 创建方式选择弹窗 ---------------- */
  openNewModal() {
    const typeList = Object.entries(STOCK_POST_TYPE_MAP).map(([k, v]) => ({ key: k, ...v }));
    const colors = [
    ...
    const cards = typeList.map((t, i) => {
      ...
      return `
        <div onclick="closeModal();SpStockPost.openForm('${t.key}')"
    ...
    const body = `
      <div style="padding:4px 0;">
        ...
        <div style="margin-top:14px;padding:8px 12px;background:#f9fafb;border-radius:8px;font-size:12px;color:var(--text-muted);text-align:center;border:1px dashed var(--border);">
          💡 提示：库存记账为直接过账（不建预留单），保存后自动调用 SAP 过账接口，返回物料凭证号后生效
        </div>
      </div>`;
    showModal('📌 选择记账类型', body, [{ text: '取消', cls: 'btn-secondary', action: closeModal }], 'modal-xl');
  },

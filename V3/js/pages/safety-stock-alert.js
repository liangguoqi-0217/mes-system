...
const SafetyStockAlert = {
      ...
      <div style="display:flex;flex-direction:column;height:calc(100vh - 56px);">
        <div style="background:linear-gradient(135deg,#d97706,#f59e0b);color:white;padding:16px 24px;display:flex;align-items:center;justify-content:space-between;flex-shrink:0;">
          ...
          <div style="display:flex;gap:8px;">
            ...
            <button class="btn btn-blue" onclick="SafetyStockAlert.exportData()">+ 导出</button>
          </div>
        </div>
        ...
        <div class="table-wrapper" style="flex:1;">
          <table class="data-table">
            <thead><tr>
              <th>工厂</th><th>物料</th><th>物料描述</th><th>非限制库存</th><th>质检库存</th><th>冻结库存</th><th>安全库存</th><th>单位</th><th>库存安全线状态</th>
            </tr></thead>
            <tbody id="ssaTableBody"></tbody>
          </table>
        </div>
        <div class="list-toolbar" style="flex-shrink:0;">
          <div class="list-info"><span class="list-count" id="ssaCount">共 ${this.filtered.length} 条</span></div>
  ...
  renderTable() {
    ...
    document.getElementById('ssaTableBody').innerHTML = page.map(row => {
  ...
  search() {
    ...
    if (matType) data = data.filter(r => r.matCode.startsWith(matType));

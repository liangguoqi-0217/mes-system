...
const MaterialMaster = {
  ...
  init() {
    ...
    if (window.QueryVariant) {
      QueryVariant.mount('material-master');
      QueryVariant.restore('material-master');
      QueryVariant.bindRecent('material-master');
    }
  ...
  search() {
    ...
    if (window.QueryVariant) {
      QueryVariant.saveAuto('material-master');
      QueryVariant.recordUsed('material-master');
    }
  },
  ...
  reset() {
    ...
    if (window.QueryVariant) QueryVariant.resetSelection('material-master');
  },
  ...
  addModal() {
    showModal(
      ...
      `<form id="matAddForm" onsubmit="return false;" style="display:grid;grid-template-columns:1fr 1fr;gap:12px 16px;">
        ...
        <div class="form-group"><label>基本单位<span class="required">*</span></label><select id="addBaseUnit" required><option value="">请选择</option><option value="个">个</option><option value="件">件</option><option value="套">套</option><option value="台">台</option><option value="支">支</option><option value="盒">盒</option><option value="瓶">瓶</option><option value="桶">桶</option><option value="箱">箱</option><option value="袋">袋</option><option value="卷">卷</option><option value="张">张</option><option value="kg">kg</option><option value="L">L</option><option value="吨">吨</option></select></div>
...
if (window.QueryVariant) {
  QueryVariant.register({
    pageId: 'material-master',
    fields: ['matFactory', 'matMrpCtrl', 'matWholesale', 'matProcType', 'matType', 'matCode'],
    textFields: ['matCode'],
    labels: { matFactory: '工厂', matMrpCtrl: 'MRP控制者', matWholesale: '批次管理', matProcType: '采购类型', matType: '物料类型', matCode: '物料号' },
    onApply: function () { MaterialMaster.search(); }
  });
}
